...
def U_double_well_hard(xs, beta=1.0):
    ...
    out = -jnp.log(
        jnp.exp(U1 - jnp.maximum(U1, U2)) + jnp.exp(U2 - jnp.maximum(U1, U2))
    ) - jnp.maximum(U1, U2)
    return beta * out
...
def U_mueller_brown(xs, beta=1.0):
    ...
    e3 = -170 * jnp.exp(
        -6.5 * (0.5 + x) ** 2 + 11 * (x + 0.5) * (y - 1.5) - 6.5 * (y - 1.5) ** 2
    )
    e4 = 15.0 * jnp.exp(
        0.7 * (1 + x) ** 2 + 0.6 * (x + 1) * (y - 1) + 0.7 * (y - 1) ** 2
    )
...
systems = {
    ...
    "double_well_dual_channel": (
        U_double_well_dual_channel,
        jnp.array([-0.5, 0]),
        jnp.array([0.5, 0]),
        jnp.array((-1.0, 1.0)),
        jnp.array((-1.0, 1.0)),
    ...
    "mueller_brown": (
        U_mueller_brown,
        jnp.array([-0.55828035, 1.44169]),
        jnp.array([0.62361133, 0.02804632]),
        jnp.array((-1.5, 0.9)),
        jnp.array((-0.5, 1.7)),
    ),
}
...
class System:
    def __init__(self, args):
        self.xi = args.xi
        self.std = args.xi * jnp.sqrt(args.timestep)
        # self.kB = 8.6173303e-5
        # self.std = jnp.sqrt(2 * self.kB * args.temperature * args.timestep)
        self.log_prob = lambda x: norm.logpdf(x, loc=0, scale=self.std)

        U, self.start_position, self.target_position, self.xlim, self.ylim = systems[
            args.system
        ]
